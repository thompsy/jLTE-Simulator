...
public class MaxCISector extends AbstractSector {
    ...
    public static MaxCISector fromXML(Configuration config, Node xml, ENodeB eNodeB, Location location) {
        ...
        return new MaxCISector(config, params.getSectorId(), eNodeB, location, params.getTxPower(), params.getAzimuth(), params.getHeight(), params.getDowntilt(), params.getAntennaGain());
    }

    private MaxCISector(Configuration config, int sectorId, final ENodeB eNodeB, final Location loc, double txPower, double azimuth, double height, double downtilt, double antennaGain) {
    ...
    protected void doDownlinkAllocation(final int iteration, final int subframe) {
        ...
        for (ResourceBlock RB : unscheduledRBs) {
            ...
            Collections.sort(toSchedule, UEComparators.getRBSignalComparator(RB.id));
